 ...
 * Date: 2015-12-14 Edit By ToNict
 */
//jsHint options
...
/*global jQuery, app, phpGrid, define, HTMLElement, HTMLTableRowElement */
...
(function(factory) {
  ...
  var englishLanguageDefaults = {
    ...
    errors: {
      ...
      cannotAddSelect: "Please select master data!"
    },
    formatter: {
      integer: {
        thousandsSeparator: ",",
  ...
  $.extend(true, jgrid, {
    ...
    icons: {
      'default': {
        ...
        nav: {
          ...
          view: "glyphicon-folder-open",
          save: "glyphicon-floppy-save",
        ...
        form: {
          ...
          required: "glyphicon glyphicon-asterisk icon-req",
          cancel: "",
        ...
        subgrid: {
          ...
          openLtr: "glyphicon-share-alt",
          openRtl: "glyphicon-share-alt"
        },
        grouping: {
          common: "fa",
          plus: "fa-plus-square",
    ...
    guiStyles: {
      bootstrap: {
        gBox: "", // ui-widget-content??? for the children of gbox
        overlay: "ui-widget-overlay",
        loading: "",
        hDiv: "", // Old:ui-state-default ui-corner-top
        hTable: "",
        colHeaders: "",
        states: {
        ...
        filterToolbar: {
          dataField: "form-control input-xs"
        },
        subgrid: {
          thSubgrid: "", // used only with subGridModel
          rowSubTable: "", // used only with subGridModel additionally to ui-subtblcell
          row: "", // class of the subgrid row, additional to ui-subgrid
          tdStart: "", // it can be with span over rownumber and multiselect columns
          tdWithIcon: "", // class of cell with +- icon, additional to subgrid-cell
          tdData: "" // class of main td with span over the grid, additional subgrid-data
        },
        grid: "",
        gridRow: "",
        rowNum: "",
        gridFooter: "",
        rowFooter: "",
        gridTitle: "", // Old:ui-widget-header ui-corner-top
        titleButton: "", // Old:ui-corner-all
        toolbarUpper: "",
        toolbarBottom: "",
        pager: "",
        pagerButton: "",
    ...
    parseDate: function(format, date, newformat, opts) {
      ...
      if (opts.isBuddhist === true) {
        if (opts.srcformat !== newformat) {
          year += 543;
        } else {
          year -= 543;
        }
      ...
      var dayOfWeek = (w + 6) % 7 + 1, // numeric representation of the day of the week. 1 (for Monday) through 7 (for Sunday)
    ...
    detectRowEditing: function(rowid) {
      var i, savedRowInfo, tr, self = this,
        rows = self.rows,
        p = self.p,
    ...
    getDataFieldOfCell: function(tr, iCol) {
      var p = this.p,
        $td = jgrid.getCell.call(this, tr, iCol);
      if (p.treeGrid && $td.children("span.tree-wrap").length > 0) {
    ...
    enumEditableCells: function(tr, mode, callback) {
      ...
      var iCol, colModel = p.colModel,
        nCol = colModel.length,
        cm, nm, options,
        isEditable, iRow = tr.rowIndex,
        td, $dataElement, dataWidth,
        frozenRows = grid.fbRows,
        frozen = frozenRows != null,
        ...
        if (nm !== "cb" && nm !== "subgrid" && nm !== "rn") {
          ...
          if (!$dataElement.hasClass("not-editable-cell")) {
            ...
            if (p.treeGrid === true && nm === p.ExpandColumn) {
              dataWidth -= $dataElement.children("span.tree-wrap").outerWidth();
    ...
    getEditedValue: function($dataFiled, cm, useTextInSelects, editable) {
      var result, checkBoxValues, newformat, $field, valuesOrTexts, selectMethod = useTextInSelects ? "text" : "val",
        ...
        nameSelector = "[name=" + jgrid.jqID(cm.name) + "]",
        $t = this,
        $self = $($t),
        infoDialog, getRes, errcap, bClose;
      if (editable === "hidden") {
      ...
      switch (cm.edittype) {
        ...
        case "select":
          ...
          break;
        case "file":
          var input = $dataFiled.find("input" + nameSelector).val();
          var $inputFile = $dataFiled.find("input[name=File" + jgrid.jqID(cm.name) + "]");
          result = [input, $inputFile.val(), $inputFile[0].files[0]];
          break;
        case "custom":
          try {
    ...
    fullBoolFeedback: function(callback, eventName) {
      var self = this,
        ...
        result = $(self).triggerHandler(eventName, args);
     ...
     */
    builderFmButon: function(id, text, icon, iconOnLeftOrRight, style) {
      var p = this.p,
      ...
      return "<button type='button' id='" + id + "' class='" +
        jgrid.mergeCssClasses("fm-button",
          getDialogGuiStyles("fmButton"),
          getDialogGuiStyles(style ? style : 'defaultButton'),
          iconOnLeftOrRight === "right" ? "fm-button-icon-right" : (iconOnLeftOrRight === "left" ? "fm-button-icon-left" : "")
        ) + "'>" +
    ...
    parseDataToHtml: function(len, ids, items, cellsToDisplay, rcnt, adjust, readAllInputData) {
      var self = this,
        p = self.p,
        $self = $(self),
        i, j, altr, cn1, selr, idr, rd, cells, iStartTrTag,
        selected = false,
        rowData = [],
        cn = (p.altRows === true) ? p.altclass : "",
        grpdata = [],
        hiderow = p.grouping ? p.groupingView.groupCollapse === true : false,
        rn = parseInt(p.rowNum, 10),
        cmName, $j = $.fn.jqGrid,
        ...
        addMulti = function(rowid, pos, irow, checked, item) {
          ...
          if (hasCbox) {
            checkboxHtml = "<label class='checkbox'><input type='checkbox'" + " id='jqg_" + p.id + "_" + rowid +
              "' class='cbox' " + //name='jqg_" + p.id + "_" + rowid + "'
              (checked ? " checked='checked' aria-checked='true'" : " aria-checked='false'") + "/><i></i></label>";
          }
  ...
  $.fn.jqGrid = function(pin) {
    ...
    return this.each(function() {
      ...
      var ts = this,
        localData, localDataStr, $self0 = $(ts),
        isFunction = $.isFunction,
        isArray = $.isArray,
        extend = $.extend,
        inArray = $.inArray,
        trim = $.trim,
        each = $.each,
        setSelection = $j.setSelection,
        getGridRes = $j.getGridRes,
        ...
        iconSet = pin.iconSet || defaults.iconSet || "default",
        guiStyle = pin.guiStyle || defaults.guiStyle || "bootstrap",
      ...
      var p = extend(true, {
            ...
            search: false,
            caption: "",
            captionicon: getIcon("other.caption"),
            hidegrid: true,
            hiddengrid: false,
            ...
            cmTemplate: {},
            idPrefix: "",
            iconSet: iconSet, // "fontAwesome" or "bootstrap" or some custom value
            guiStyle: guiStyle,
            locale: locale,
            multiSort: false,
            autoResizable: true,
            gridLines: "both", // Add both, horizontal, vertical, none
            whiteSpacePre: true,
            globalId: "", // plugin global id
            parentId: "", // plugin parent id
            tabId: "", // plugin tabs panel id
            tabsView: {
              labelField: [],
              labelText: ""
            },
            tableDnd: false, // jquery.tablednd.js
            gridDetail: {}, // Grid Master/Detail
            treeIcons: {
      ...
      var propOrAttr = p.propOrAttr,
        ...
        buildAddPropMap = function(additionalProperties) {
          var m = {},
            i, n = additionalProperties.length,
            addPropInfo;
          for (i = 0; i < n; i++) {
        ...
        buildArrayReader = function() {
          var i, colModel = p.colModel,
            cmNamesInputOrder = p.cmNamesInputOrder,
            additionalProperties = p.additionalProperties,
            n = cmNamesInputOrder.length,
            arrayReaderInfos,
            name, index, order;
          p.arrayReaderInfos = {};
              ...
              if (index !== undefined) {
                ...
                if (index !== undefined) {
                  arrayReaderInfos[name] = {
                    ...
                    order: order,
                    type: 1
                  }; // INPUT_NAME_TYPE.ADDITIONAL_PROPERTY
                } else if (name === (p.prmNames.rowidName || "rowid")) {
                  arrayReaderInfos[name] = {
                    index: index,
                    type: 2
                  }; // INPUT_NAME_TYPE.ROWID
                }
              }
            }
          }
        ...
        grid = {
          ...
          dragStart: function(i, x, y, $th) {
            var self = this,
              $bDiv = $(self.bDiv),
              gridOffset = $bDiv.closest(p.gBox).offset(),
          ...
          dragMove: function(x) {
            ...
            if (resizing) {
              var diff = x.pageX + resizing.delta - resizing.startX,
          ...
          scrollGrid: function(e) { // this must be bDiv
            var bDiv = this,
              $bTable = getGridComponent(COMPONENT_NAMES.BODY_TABLE, bDiv);
            if (e) {
              e.stopPropagation();
            }
            if ($bTable.length === 0) {
            ...
            if (p.scroll) {
              ...
              if (gridSelf.scrollTop === undefined) {
                gridSelf.scrollTop = 0;
              }
              if (scrollTop !== gridSelf.scrollTop) {
                ...
                gridSelf.timer = setTimeout(function() {
                  gridSelf.populateVisible.call($bTable[0]);
                }, p.scrollTimeout);
              }
            }
      ...
      var sortkeys = ["shiftKey", "altKey", "ctrlKey"],
        ...
        normalizeRemapColumns = function() {
          ...
          var remapColumns = p.remapColumns,
            colModel = p.colModel,
            nCol = colModel.length,
            cmNames = [],
            i, remappedCmNames,
        ...
        formatCol = function(pos, rowInd, tv, rawObject, rowId, rdata) {
          var cm = p.colModel[pos],
            cellAttrFunc, cellValue = tv,
            rPrefix,
              ...
              while (true) {
                ...
                if (matches === null || matches.length < 5) {
                  ...
                  return rest + " style='" + encodeAttr(styleValue) + "'" +
                    ...
                    (tilteValue && cm.hidden !== true ? " title='" + encodeAttr(tilteValue) + "'" : "");
                }
                value = matches[3];
                attrStr = matches[4];
        ...
        formatter = function(rowId, cellval, colpos, rwdat, act, rdata) {
          ...
          if (p.treeGrid && act !== "edit" && ((p.ExpandColumn === undefined && colpos === 0) || (p.ExpandColumn === cm.name))) {
            ...
            var curLevel = parseInt(rdata[p.treeReader.level_field] || 0, 10),
              levelOffset = 19,
            ...
            v = "<span class='tree-wrap' style='width:" + ((lftpos + 1) * levelOffset) +
              "px;'><span class='" +
              ...
              (lftpos * levelOffset) + "px;'></span></span>" +
        ...
        normalizeData = function() {
          var data = p.data,
            ...
            i, cur, cells, idName, idIndex, v, rd, id,
            localReader = p.localReader,
            additionalProperties = p.additionalProperties,
            cellName = localReader.cell,
            cmName, isArrayCells, addProp, info,
            arrayReaderInfos = p.arrayReaderInfos;
        ...
        readInput = function(data, rcnt, more, adjust) {
          var self = this,
            ...
            locid = "_id_",
            dataReader = p.localReader,
            fieldReader = getAccessor;
          ...
          var i, cells, len, drows, idName, idIndex, rd = {},
            idr,
            colModel = p.colModel,
            nCol = colModel.length,
            cm, cmName,
            ...
            nodeReader = function(nodeName) {
              return function() {
                ...
                /*var elem = obj.getElementsByTagName(nodeName)[0], childNodes;
                 ...
                 return undefined;*/
                var elem = preloadedNodes[nodeName],
                  childNodes;
                if (elem == null) {
                  return undefined;
                }
          ...
          var colReader = {},
            nameReader, isArrayCells, v, addProp, items,
            additionalProperties = p.additionalProperties,
          ...
          if (p.reccount > 0) {
            $(".ui-jqgrid-nodata", "#" + p.gViewId).hide();
            $(".ui-jqgrid-bdiv", "#" + p.gViewId).css("-webkit-overflow-scrolling", "touch");
            base.autoResize.call($self);
          } else {
            $(".ui-jqgrid-nodata", "#" + p.gViewId).show();
          }
        },
        addLocalData = function() {
          ...
          var grpview = p.grouping ? p.groupingView : false,
            lengrp, gin;
        ...
        updatepager = function(rn, dnd) {
          ...
          if (p.toolbar[0]) {
            if ((p.multiPageSelection && p.selarrrow.length === 0) || !p.multiPageSelection) {
              $('.selectable', '#t_' + p.id).prop("disabled", true);
            }
          }

          if (p.treeGrid) {
            $self.setTreeMoveNode();
          }
        ...
        beginReq = function() {
          ...
          $j.progressBar.call($(self), {
            method: "show",
            loadtype: p.loadui,
            htmlcontent: "<span class='" + getIcon("other.loading") + "'></span> " + getDef("loadtext")
          });
        },
        endReq = function() {
          var self = this;
        ...
        populate = function(npage) {
          ...
          if (!gridSelf.hDiv.loading) {
            ...
            if (p.treeGrid === true && pN.btnongrid !== null) {
              prm[pN.btnongrid] = p.btnongrid;
            }

            var lcf = isFunction(p.loadComplete),
              lc = lcf ? p.loadComplete : null;
            var adjust = 0;
            npage = npage || 1;
            if (npage > 1) {
              if (pN.npage !== null) {
            ...
            var rcnt = !p.scroll ? 1 : self.rows.length - 1,
              fixDisplayingHorizontalScrollbar = function() {
                ...
                var gBodyWidth = $self.width(),
                  gViewWidth = $self.closest(".ui-jqgrid-view").width(),
        ...
        setPager = function(pgid, tp) {
          var disabledClasses = getGuiStyles("states.disabled"),
            /* hoverClasses = getGuiStyles("states.hover"),  */
          ...
          if (p.pginput === true) {
            pginp = "<td dir='" + dir + "'>" + jgrid.format(getDef("pgtext") || "", "<input class='" + getGuiStyles("pagerInput", "ui-pg-input") + "' type='text' size='2' maxlength='7' value='0'/>", "<span id='sp_1_" + pgid + "'>0</span>") + "</td>";
          }
        ...
        multiSort = function(iCol, obj) {
          ...
          for (var i = 0, len = sortarr.length; i < len; i++) {
            ...
            p.sortname += sortarr[i];
            if (i !== len - 1) {
              p.sortname += " " + sortord[i];
            }
          }
          p.sortorder = sortord[len - 1];
        },
        ...
        setInitialColWidth = function() {
          var initialWidth = 0,
            borderAndPaddingWidth = jgrid.cell_width ? 0 : intNum(p.cellLayout, 0),
            numberOfVariableColumns = 0,
            iLastVariableColumn, scrollbarWidth = intNum(p.scrollOffset, 0),
            columnWidth, hasScrollbar = false,
            totalVariableWidth, fixedColumnsWidth = 0,
            correctur,
      ...
      if (p.multiselect && (p.multiselectPosition === "left" || p.multiselectPosition === "right")) {
        ...
        p.colNames[insertMethod]("<label class='checkbox'><input id='" + p.cbId + "' class='cbox' type='checkbox' aria-checked='false'/><i></i></label>");
      ...
      var idn, w, res, sort, cmi, tooltip, labelStyle, ptr, sortarr = [],
        sortord = [],
        sotmp = [],
      ...
      for (iCol = 0; iCol < p.colNames.length; iCol++) {
        ...
        headerText = (cmi.name !== 'rn' && cmi.name !== 'cb') ?
          "<span " + (cmi.autoResizable && cmi.formatter !== "actions" ? "class='" + p.autoResizing.wrapperClassName + "' " : '') + (labelStyle === "" ? "" : " style='" + labelStyle + "'") + ">" + p.colNames[iCol] + "</span>" :
          p.colNames[iCol];
        if (p.sortIconsBeforeText) {
          ...
          thead += headerText;
          thead += (cmi.name !== 'rn' && cmi.name !== 'cb') ? (p.builderSortIcons || jgrid.builderSortIcons).call(ts, iCol) : '';
        }
        thead += "</div></th>";
        tbody += "<td></td>";
      ...
      if (p.multiselect) {
        $(p.cb, hTable).bind("click", function() {
          var highlightClass = getGuiStyles("states.select"),
            ...
            selectUnselectRow = function(tr, toSelect) {
              ...
              if (iColCb !== undefined) { // p.multiselectCheckboxes
                $(tr.cells[iColCb]).find("input.cbox")[p.propOrAttr]("checked", toSelect);
              }
            },
            frozenRows = grid.fbRows,
          ...
          if (p.toolbar[0]) {
            $('.selectable', '#t_' + p.id).prop("disabled", !toCheck);
          }
      ...
      var tfoot = "";
      if (p.footerrow) {
        tfoot += "<table role='presentation' style='width:" + p.tblwidth + "px' class='ui-jqgrid-ftable'" + (isMSIE7 ? " cellspacing='0'" : "") + "><tbody><tr role='row' class='" + getGuiStyles("rowFooter", "footrow footrow-" + dir) + "'>";
      }
      ...
      $("th", hTable[0].tHead.rows[0]).each(function(j) {
        ...
        var classes = "";
        if (cm.onTablet === false) {
          $th.addClass("visible-lg visible-md");
          classes += " visible-lg visible-md";
        }
        if (cm.onMobile === false) {
          $th.addClass("hidden-xs");
          classes += " hidden-xs";
        }

        if (classes) {
          cm.classes = cm.classes ? cm.classes + classes : classes;
        }
        ...
        var $th = $(this),
          isFrozen = $th.closest(".ui-jqgrid-hdiv").hasClass("frozen-div"),
          getOffset = function() {
      ...
      switch (p.gridLines) {
        case "horizontal":
          $self0.addClass("ui-jqgrid-line-horizontal");
          break;
        case "vertical":
          $self0.addClass("ui-jqgrid-line-vertical");
          break;
        case "none":
          $self0.addClass("ui-jqgrid-line-none");
          break;
      }

      if (p.whiteSpacePre === false) {
        $self0.addClass("ui-jqgrid-whitespace-normal");
      }
      ...
      if (p.pager) {
        ...
        if ($pager.length > 0) {
          $pager.css({
            width: grid.width + "px"
          }).addClass(getGuiStyles("pager", "ui-jqgrid-pager")).appendTo(eg);
          if (hg) {
            $pager.hide();
          }
      ...
      $self0.before(grid.hDiv)
        .click(function(e) {
          ...
          if (target.tagName === "A" || (locked && !scb) || target.tagName === "I" || $(target).hasClass("checkbox") || ($(target).hasClass("treeclick") && e.which === 1)) {
            return;
          }
          ci = $td[0].cellIndex;
          tdHtml = $td.html();
        ...
        .dblclick(function(e) {
          var $td = getTdFromTarget.call(this, e.target),
            $tr = $td.parent();
      ...
      $(grid.bDiv)
        .append($("<div style='position:relative;" + (isMSIE7 ? "height:0.01%;" : "") + "'></div>").append(ts))
      ...
      if (p.pager) {
        $(grid.bDiv).css("margin-bottom", "-1px");
      }
      if (p.maxHeight) {
      ...
      $(grid.bDiv).append('<div class="ui-jqgrid-nodata"><span class="' + getIcon("other.noDataDisplay") + '"></span><strong>&nbsp; ' + getDef("nodata") + '</strong></div>');
      if (!$.support.tbody) { //IE
      ...
      var visibleGridIcon = getIcon("gridMinimize.visible"),
        hiddenGridIcon = getIcon("gridMinimize.hidden"),
        ...
        toolbarContainer = "<div class='toolbar-container'><div class='swiper-wrapper'></div></div>";
      $(grid.cDiv).append("<span class='ui-jqgrid-title'>" + (p.captionicon ? "<span class='" + p.captionicon + "'></span> " : '') + p.caption + "</span>")
        .append(arf)
      ...
      if (p.toolbar[0]) {
        ...
        if (p.toolbar[1] === "both") {
          ...
          $(grid.uDiv).addClass(toolbarUpperClasses).attr("id", "t_" + p.id).append(toolbarContainer).insertBefore(grid.hDiv);
          $(grid.ubDiv).addClass(getGuiStyles("toolbarBottom", "ui-userdata")).attr("id", "tb_" + p.id).append(toolbarContainer).insertAfter(grid.hDiv);
          if (hg) {
            $(grid.ubDiv).hide();
          }
        } else {
          $(grid.uDiv).width(grid.width).addClass(toolbarUpperClasses).attr("id", "t_" + p.id).append(toolbarContainer);
        }
        if (hg) {
          $(grid.uDiv).hide();
        }
      ...
      if (p.toppager) {
        ...
        $(grid.topDiv).addClass(getGuiStyles("pager", "ui-jqgrid-toppager")).css({
          width: grid.width + "px"
  ...
  jgrid.extend({
    ...
    getGuiStyles: function(path, jqClasses) {
      ...
      var p = $t.p,
        guiStyle = p.guiStyle || jgrid.defaults.guiStyle || "bootstrap";
    ...
    setSelection: function(selection, onsr, e) {
      return this.each(function() {
        var $t = this,
          $self = $($t),
          p = $t.p,
          stat, pt, ner, ia, tpsr, csr, $tr,
          getGuiStyles = base.getGuiStyles,
          ...
          frozenRows = $t.grid.fbRows,
          selectUnselectRow = function(tr1, toSelect) {
            var method = toSelect ? "addClass" : "removeClass",
              iColCb = p.iColByName.cb,
              attributes = toSelect ? {
                "aria-selected": "true",
                tabindex: "0"
        ...
        if (!p.multiselect) {
          if (pt.className !== "ui-subgrid") {
            ...
            if (onsr) {
              ...
              if (p.toolbar[0]) {
                $('.selectable', '#t_' + p.id).prop("disabled", !stat);
              }
            }
          }
        } else {
          ...
          if (onsr) {
            feedback.call($t, "onSelectRow", pt.id, stat, e);
            if (p.toolbar[0]) {
              $('.selectable', '#t_' + p.id).prop("disabled", !p.selrow);
            }
          }
        }

        /* GridDetail */
        if (p.selrow && p.gridDetail.length > 0) {
          for (var i = 0; i < p.gridDetail.length; i++) {
            $("#" + p.gridDetail[i].gridId).jqGrid("setGridParam", {
              postData: {
                id: p.selrow
              },
              parentId: p.selrow
            }).trigger("reloadGrid");
            app.removeTabAll(p.gridDetail[i].tabId);
          }
        }
        ...
        var $t = this,
          ...
          deselectRow = function(tr) {
            ...
            if (multiselectChechboxes) { // p.multiselect or p.multiselectCheckboxes
              $(tr.cells[iColCb]).find("input.cbox")[p.propOrAttr]("checked", false);
            }
            if (frozenRows) {
              frozenRow = frozenRows[tr.rowIndex];
              $(frozenRow)[method](highlightClass).attr(attributes);
              if (multiselectChechboxes) { // p.multiselect or p.multiselectCheckboxes
                $(frozenRow.cells[iColCb]).find("input.cbox")[p.propOrAttr]("checked", false);
              }
            }
          };
        if (rowid !== undefined) {
          ...
          if ($(ind).hasClass("jqgrow")) {
            ...
            for (i = 0; i < $td.length; i++) {
              ...
              if (nm !== "cb" && nm !== "subgrid" && nm !== "rn" && cm.formatter !== "actions" && (!options.skipHidden || !cm.hidden)) {
                td = $td[i];
                if (p.treeGrid === true && nm === p.ExpandColumn) {
                  res[nm] = htmlDecode($("span", td).last().html());
                } else {
                  try {
    ...
    delRowData: function(rowid) {
      ...
      this.each(function() {
        ...
        if (p.records === 0) { /* && p.treeGrid === false */
          $(".ui-jqgrid-nodata", "#" + p.gViewId).show();
        }
      });
      return success;
        ...
        if (data) {
          try {
            ...
            $(p.colModel).each(function(i) {
              var cm = this,
                nm = cm.name,
                title, dval = getAccessor(data, nm);
              if (dval !== undefined) {
    ...
    addRowData: function(rowid, rdata, pos, src) {
      ...
      var success = false,
        nm, row, sind, i, v, aradd, cnm, cn, data, cm, id;
      if (rdata) {
        if ($.isArray(rdata)) {
          aradd = true;
          //pos = "last";
        ...
        this.each(function() {
          ...
          while (k < datalen) {
            ...
            for (i = 0; i < additionalProperties.length; i++) {
              addProp = additionalProperties[i];
              if (addProp !== null && $.isFunction(addProp.convert)) {
                v = getAccessor(data, addProp.name);
                if (v !== undefined) {
                  lcdata[addProp.name] = addProp.convert(v);
                }
              } else {
                v = getAccessor(data, addProp);
                if (v !== undefined) {
                  lcdata[addProp] = v;
                }
              }
    ...
    showHideCol: function(colname, show) {
      return this.each(function() {
        var $t = this,
          ...
          fndh = false,
          p = $t.p,
          brd = jgrid.cell_width ? 0 : p.cellLayout,
          cw;
        if (!grid) {
          return;
        }
        ...
        $(p.colModel).each(function(i) {
          if ($.inArray(this.name, colname) !== -1 && this.hidden === sw) {
            ...
            if (p.footerrow) {
              $("tr.footrow td:eq(" + i + ")", grid.sDiv).css("display", show);
            }
    ...
    hideCol: function(colname) {
      return this.each(function() {
        base.showHideCol.call($(this), colname, "none");
      });
    },
    showCol: function(colname) {
      return this.each(function() {
        base.showHideCol.call($(this), colname, "");
      });
    },
    ...
    remapColumnsByName: function(permutationByName, updateCells, keepHeader) {
      var ts = this[0],
        p = ts.p,
        permutation = [],
        i, n, cmNames = permutationByName.slice(),
        inArray = $.inArray;
    ...
    setGridWidth: function(newWidth, shrink) {
      return this.each(function() {
        var $t = this,
          p = $t.p,
          columnWidth, grid = $t.grid,
          initialWidth = 0,
          iLastVariableColumn, numberOfVariableColumns = 0,
          hasScrollbar = false,
          totalVariableWidth, fixedColumnsWidth = 0,
          correctur,
          isCellClassHidden = jgrid.isCellClassHidden,
          newGridWidth = newWidth,
          resizable = true;
        if (!grid || p == null) {
          return;
        }
        ...
        var colModel = p.colModel,
          cm, scrollbarWidth = p.scrollOffset,
          borderAndPaddingWidth = jgrid.cell_width ? 0 : p.cellLayout,
          thInfo,
          headers = grid.headers,
          footers = grid.footers,
          bDiv = grid.bDiv,
          hDiv = grid.hDiv,
          sDiv = grid.sDiv,
          cols = grid.cols,
          delta, colsExist, shrinkFactor,
        ...
        if (shrink === true) {
          ...
          if (initialWidth > (p.width + 20)) { /* numberOfVariableColumns === 0 || */
            resizable = false;
          }
          ...
          if (resizable === true) {
            ...
            $.each(colModel, function(i) {
              if (this.hidden === false && !isCellClassHidden(this.classes) && !this.fixed) {
                ...
                if (this.frozen === true) {
                  $("th:eq(" + i + ")", $(grid.fhDiv)).css('width', columnWidth + 'px');
                  $("tr:first td:eq(" + i + ")", $(grid.fbDiv)).css('width', columnWidth + 'px');
                }
    ...
    setCell: function(rowid, colName, nData, cssp, attrp, forceUpdate) {
      ...
      return this.each(function() {
        var $t = this,
          ...
          iCol = -1,
          colModel = p.colModel,
          v, title, i, cm, item, tr, $td, $tdi, val, rawdat = {},
          id, index;
        if (!$t.grid) {
          return;
        }
          ...
          var tr = $(this).find("tr[tabindex=0]")[0],
            ...
            moveHorizontal = function(stringLeftOrRight) {
              if (p.treeGrid) {
                ...
                if (expanded) {
                  base.resetSelection.call($self);
                  $(tr).find("span.treeclick").trigger("click");
                }
              }
    ...
    getAutoResizableWidth: function(iCol) {
      ...
      self = self[0];
      var rows = self.rows,
        row, cell, iRow, $cell, $cellFirstChild,
        p = self.p,
        cm = p.colModel[iCol],
    ...
    autoResize: function() {
      return this.each(function() {
        var self = this,
          ...
          p = self.p,
          pw, nw;
        if (!self.grid || p.autoResizable === false) {
          return;
        }

        pw = parseInt($(p.gBox).parent().width() - 2);
        nw = parseInt(pw > p.maxwidth && p.maxwidth > 0 ? p.maxwidth : pw);
        if (p.width !== nw) {
          base.setGridWidth.call($self, nw);
        }
      });
    }
  });
  ...
  jgrid.extend({
    ...
    saveCell: function(iRow, iCol) {
      return this.each(function() {
        ...
        var errors = $self.jqGrid("getGridRes", "errors"),
          errcap = errors.errcap,
          edit = $self.jqGrid("getGridRes", "edit"),
          bClose = edit.bClose,
          savedRow = p.savedRow,
    ...
    GridNav: function() {
      return this.each(function() {
        ...
        $("#" + p.knv)
          .focus()
          .keydown(function(e) {
            var iRowOld = parseInt(p.iRow, 10),
              iColOld = parseInt(p.iCol, 10);
            kdir = e.keyCode;
            if (p.direction === "rtl") {
              if (kdir === 37) {
        ...
        $(rows).each(function(j) {
          ...
          if ($(this).hasClass("edited")) {
            ...
            $(this.cells).each(function(i) {
              var cm = p.colModel[i],
                nm = cm.name,
                $td = getTdByColumnIndex.call($t, tr, i); // $td = $(this);
              if (nm !== "cb" && nm !== "subgrid" && nm !== "rn" && (mthd !== "dirty" || $td.hasClass("dirty-cell"))) {
                try {
                  res[nm] = $.unformat.call($t, $td[0], {
                    rowId: rows[j].id,
                    colModel: cm
                  }, i);
                } catch (e) {
  ...
  $.extend(jgrid, {
    ...
    createModal: function(aIDs, content, o, insertSelector, appendsel, css, size) {
      var jqID = jgrid.jqID,
        p = this.p,
        gridjqModal = p != null ? p.jqModal || {} : {};
      o = $.extend(true, jgrid.jqModal || {}, gridjqModal, o);
      ...
      var mw = document.createElement("div"),
        themodalSelector = "#" + jqID(aIDs.themodal),
        rtlsup = $(o.gbox).attr("dir") === "rtl" ? true : false;

      css = $.extend({}, css || {});
      mw.className = "modal fade";
      mw.id = aIDs.themodal;
      mw.dir = rtlsup ? "rtl" : "ltr";
      mw.tabIndex = "-1";
      var md = document.createElement('div');
      md.className = "modal-dialog" + (size ? ' ' + size : '');
      $(mw).prepend(md);
      ...
      mh.className = "modal-header " + (rtlsup ? "ui-jqdialog-titlebar-rtl" : "ui-jqdialog-titlebar-ltr");
      mh.id = aIDs.modalhead;

      var ahr = $("<span class='" + jgrid.getIconRes(p.iconSet, "form.close") + " close' aria-hidden='true' data-dismiss='modal'></span>");

      $(mh).append(ahr);
      $(mh).append("<h4 class='modal-title'>" + o.caption + "</h4>");
      // create "div.ui-jqdialog-content" which hold some HTML content (see input parameter)
      var mc = document.createElement("div");
      $(mc).addClass("modal-content").attr("id", aIDs.modalcontent);
      $(mc).append(content);
    ...
    viewModal: function(selector, o) {
      ...
      if (o.caption !== undefined) {
        $(selector).find(".modal-title").html(o.caption);
      }
    ...
    info_dialog: function(caption, content, closeButtonText, modalopt) {
      var $t = this,
        ...
        gridjqModal = p != null ? p.jqModal || {} : {},
        ids = {
          themodal: "infomod" + p.id,
          modalhead: "infohd" + p.id,
          modalcontent: "infocnt" + p.id
        },

        themodalSelector = "#" + jgrid.jqID(ids.themodal),
      ...
      var buttstr = "",
        i; //, hoverClasses = getGuiStyles.call($t, "states.hover");
      ...
      if ($(themodalSelector).length === 0) {

        var cnt = "<div class='modal-body'>";
        cnt += "<div class='infocnt' style='" + cn + "'>" + content + "</div>";
        cnt += "</div>";
        if (closeButtonText || buttstr !== "") {
          cnt += "<div class='modal-footer' style='text-align:" + mopt.buttonalign + "'>" +
            (closeButtonText ? jgrid.builderFmButon.call($t, "closedialog", closeButtonText) : "") + buttstr + "</div>";
        }

        jgrid.createModal.call($t, {
          themodal: ids.themodal,
          modalhead: ids.modalhead,
          modalcontent: ids.modalcontent
        }, cnt, mopt, p.gView);
        // attach onclick after inserting into the dom
        if (buttstr) {
          $.each(mopt.buttons, function(j) {
            $("#" + jgrid.jqID($t.id), themodalSelector).bind("click", function() {
              mopt.buttons[j].onClick.call($("#info_dialog"));
              return false;
            });
          });
        }
        $("#closedialog", themodalSelector).click(function() {
          jgrid.hideModal(themodalSelector, {
            gb: p.gBox,
            jqm: jm,
            onClose: mopt.onClose,
            removemodal: mopt.removemodal
          });
          return false;
        });
      } else {
        var addinfocnt = true;
        $(".infocnt", themodalSelector).each(function() {
          if ($(this).text() === content) {
            addinfocnt = false;
          }
        });
        if (addinfocnt === true) {
          $(".modal-body", themodalSelector).empty().append("<div class='infocnt' style='" + cn + "'>" + content + "</div>");
        }
      }
    ...
    createEl: function(eltype, options, vl, autowidth, ajaxso) {
      ...
      var i, so, sv, ov, oSv, key, optionInfos = [],
        optionInfo,
        sep = options.separator === undefined ? ":" : options.separator,
        delim = options.delimiter === undefined ? ";" : options.delimiter,
        emptyOptionText = options.emptyOptionText ? options.emptyOptionText : getRes("defaults.pleaseSelect"),
      ...
      function setAttributes(elm, atr, exl) {
        var exclude = ["dataInit", "dataEvents", "dataUrl", "buildSelect", "sopt", "searchhidden", "defaultValue", "attr", "custom_element", "custom_value", "selectFilled", "rowId", "mode", "oper", "maxitems", "url", "path", "emptyOptionText"];
      ...
      switch (eltype) {
        ...
        case "radio":
          ...
          if (options.value) {
            ...
            for (i = 0; i < optionInfos.length; i++) {
              ...
              setAttributes(radioButton, options, ["value", "id"]);
              radioButton.type = 'radio';
              radioButton.value = optionInfo.value;
              if (optionInfo.value === vl) {
                radioButton.checked = true;
              } else {
                radioButton.checked = false;
              }

              $(elem).append(label.append(radioButton).append(optionInfo.innerHtml));
            }
          }
        ...
        case "file":
          elem = document.createElement("input");
          elem.type = eltype;
          options.name = "File" + options.name;
    ...
    checkValues: function(val, valref, customobject, nam) {
      var edtrul, nm, dft, g = this,
        p = g.p,
        colModel = p.colModel,
        cm, isEmpty = jgrid.isEmpty,
      ...
      if (edtrul) {
        if (!nm) {
          nm = p.colNames != null ? p.colNames[valref] : cm.label;
        }
        ...
        // force required
        var rqfield = edtrul.required === true ? true : false;
  ...
  jgrid.extend({
    ...
    clearBeforeUnload: function() {
      return this.each(function() {
        var self = this,
          p = self.p,
          grid = self.grid,
          propOrMethod, clearArray = jgrid.clearArray,
    ...
    setGridState: function(state) {
      return this.each(function() {
        ...
        var iconSet = p.iconSet || jgrid.defaults.iconSet || "default",
    ...
    filterToolbar: function(oMuligrid) {
      ...
      return this.each(function() {
        ...
        var o = $.extend(true, {
            ...
            resetIcon: "glyphicon glyphicon-remove",
          ...
          triggerToolbar = function() {
            ...
            $.each(colModel, function() {
              var cm = this,
                nm = cm.index || cm.name,
                v, so, searchoptions = cm.searchoptions || {},
              ...
              if (o.searchOperators) {
                so = $elem.parent().prev().children("span").data("soper") || o.defaultSearch;
              } else {
          ...
          buildRuleMenu = function(elem, left, top) {
            ...
            var selclass, ina, i = 0,
              aoprs = [],
              selected = $(elem).data("soper"),
              nm = $(elem).data("colname"),
              // fs = $(".ui-jqgrid-view").css("font-size") || "11px",
              str = '<ul id="sopt_menu" class="ui-search-menu" role="menu" tabindex="0" style="left:' + left + "px;top:" + top + 'px; display: block;">';
            i = p.iColByName[nm];
            if (i === undefined) {
              return;
            }
            var cm = colModel[i],
              ...
              odataItem, item, itemOper, itemOperand, itemText;
            if (!options.sopt) {
              options.sopt = [];
            ...
            for (i = 0; i < options.sopt.length; i++) {
              ...
              if (ina !== -1) {
                ...
                selclass = selected === itemOper ? "active" : "";
                str += '<li class="ui-menu-item ' + selclass + '" role="presentation"><a tabindex="0" role="menuitem" value="' + htmlEncode(itemOper) + '" data-oper="' + htmlEncode(itemOperand) + '">' + htmlEncode(itemOperand) + ' ' + htmlEncode(itemText) + '</a></li>';
              }
            }
            str += "</ul>";
            $("body").append(str);
            $("#sopt_menu").addClass("dropdown-menu");
            $("#sopt_menu > li > a")
              .click(function() {
        ...
        $.each(colModel, function(ci) {
          var cm = this,
            soptions, mode = "filter",
            surl, self, select = "",
            sot, so, i, searchoptions = cm.searchoptions,
            editoptions = cm.editoptions,
            formatoptions = cm.formatoptions,
            th = $("<th></th>", {
              "class": getGuiStyles.call($t, "colHeaders", "ui-th-column ui-th-" + p.direction + " " + (o.applyLabelClasses ? cm.labelClasses || "" : ""))
            }),
            thd = $("<div></div>"), //style='position:relative;height:auto;'
          ...
          if (this.search) {
            if (o.searchOperators) {
              ...
              select = "<span title='" + st + "' data-soper='" + so + "' class='soptclass' data-colname='" + this.name + "'>" + sot + "</span>";
            }
            ...
            if (soptions.clearSearch === undefined) {
              soptions.clearSearch = true; //this.stype === "text" ? true : false;
            }
            if (soptions.clearSearch) {
              var csv = getRes("search.resetTitle") || "Clear Search Value";
              $("td", stbl).eq(2).append("<span title='" + csv + "' class='clearsearchclass " + o.resetIcon + "'></span>");
            } else {
              $("td", stbl).eq(2).hide();
            }
            switch (this.stype) {
                ...
                if (surl) {
                  ...
                  $.ajax($.extend({
                    ...
                    success: function(data, textStatus, jqXHR) {
                      var cm1 = this.cm,
                        iCol1 = this.iCol,
                        soptions1 = this.options,
                        d,
                        $td = this.stbl.find(">tbody>tr>td.ui-search-input"),
                        $select;
                  ...
                  if (searchoptions) {
                    ...
                    oSv = formatoptions.value === undefined ? "" : formatoptions.value;
                    ...
                    if (typeof oSv === "string") {
                      ...
                      for (k = 0; k < so.length; k++) {
                        ...
                        if (k === 0 && sv[0] !== '') {
                          ov = document.createElement("option");
                          ov.value = '';
                          ov.innerHTML = getRes("search.emptyOptionText");
                          elem.appendChild(ov);
                        }
              ...
              case "text":
                ...
                $("td", stbl).eq(1).append("<input type='text' class='" + dataFieldClass + "' name='" + (cm.index || cm.name) + "' id='gs_" + cm.name + "' value='" + df + "'/>"); //style='width:100%;padding:0;'
                $(thd).append(stbl);
    ...
    setGroupHeaders: function(o) {
      ...
      return this.each(function() {
        ...
        var ts = this,
          i, cmi, skip = 0,
          $tr, $colHeader, th, $th, thStyle, iCol, cghi, numberOfColumns, titleText, cVisibleColumns,
          p = ts.p,
          colModel = p.colModel,
          cml = colModel.length,
          ths = ts.grid.headers,
          $theadInTable, thClasses,
    ...
    setFrozenColumns: function() {
      return this.each(function() {
            ...
            sortable: {
              options: {
                items: frozenIds.length > 0 ?
                  ">th:not(:has(" + frozenIds.join(",") + "),:hidden)" : ">th:not(:hidden)"
              }
            }
          });
        ...
        if (maxfrozen >= 0 && frozen) {
          ...
          grid.fhDiv = $('<div style="' + (p.direction === "rtl" ? "right:0;" : "left:0;") + 'top:' + top + 'px;height:' + hth + 'px;" class="' + getGuiStyles.call($t, "hDiv", "frozen-div ui-jqgrid-hdiv") + '"></div>');
          grid.fbDiv = $('<div style="' + (p.direction === "rtl" ? "right:0;" : "left:0;") + 'top:' + (parseInt(top, 10) + parseInt(hth, 10) + 1) + 'px;" class="frozen-bdiv ui-jqgrid-bdiv"></div>');
          ...
          if (p.footerrow) {
            ...
            grid.fsDiv = $("<div style='" + (p.direction === "rtl" ? "right:0;" : "left:0;") + "top:" + (parseInt(top, 10) + parseInt(hth, 10) + parseInt(hbd, 10) + 1) + "px;' class='frozen-sdiv ui-jqgrid-sdiv'></div>");
          ...
          var safeHeightSet = function($elem, newHeight) {
            ...
            fixDiv = function($hDiv, hDivBase, iRowStart, iRowEnd) {
              var iRow, n, $frozenRows, $rows, $row, $frozenRow, posFrozenTop, height, newHeightFrozen, td,
                posTop = $(hDivBase).position().top,
                frozenTableTop, tableTop, cells;
  ...
  */
  ...
  jgrid.extend({
    ...
    groupingPrepare: function(record, irow) {
      this.each(function() {
        var $t = this,
          ...
          lastvalues = grp.lastvalues,
          isInTheSameGroup = grp.isInTheSameGroup,
    ...
    groupingToggle: function(hid, clickedElem) {
      this.each(function() {
        var $t = this,
          p = $t.p,
          grp = p.groupingView,
          minusClasses = grp.minusicon,
          plusClasses = grp.plusicon,
          $tr = clickedElem ?
    ...
    groupingRender: function(grdata, rn) {
      ...
      var str = "",
        ...
        hid, clid,
        grp = p.groupingView,
        sumreverse = $.makeArray(grp.groupSummary),
      ...
      function buildSummaryTd(iEndGroup, offset, g, foffset, iconHtml) {
        var fdata = groups[iEndGroup],
          i, groupCount, strTd = "",
          tmpdata, colSpan, align, vv,
          madeHidden, nMakeHidden = 0,
          iSummary, summary, cm, iCol, summaryType, summaryTpl,
      ...
      this.unbind(eventNames)
        .bind(eventNames, function() { //e, show, cmName, iColShow) {
          ...
          var iCol = p.iColByName[grp.iconColumnName],
            iRow, row, iColNew, i; //$cellData;
        ...
        var $t = this,
          xmlConvert = function(xml, o) {
            var cnfg = $(o.xmlGrid.config, xml)[0],
              xmldata = $(o.xmlGrid.data, xml)[0],
              jstr, jstr1, key, svdatatype;
  ...
  jgrid.extend({
      ...
      return this.each(function() {
        ...
        if (($(ind).attr("editable") || "0") === "0" && !$(ind).hasClass("not-editable-row")) {
          ...
          jgrid.enumEditableCells.call($t, ind, $(ind).hasClass("jqgrid-new-row") ? "add" : "edit", function(options) {
            var cm = options.cm,
              $dataFiled = $(options.dataElement),
              dataWidth = options.dataWidth,
              tmp, opt, elc,
              nm = cm.name,
              edittype = cm.edittype,
              iCol = options.iCol,
            ...
            try {
              tmp = $.unformat.call(this, options.td, {
                rowId: rowid,
                colModel: cm
              }, iCol);
            } catch (_) {
    ...
    saveRow: function(rowid, successfunc, url, extraparam, aftersavefunc, errorfunc, afterrestorefunc, beforeSaveRow) {
      ...
      var args = $.makeArray(arguments).slice(1),
        ...
        $self = $($t),
        p = $t != null ? $t.p : null,
        editOrAdd, infoDialog = jgrid.info_dialog;
      if (!$t.grid || p == null) {
        return;
      }
      ...
      var tmp = {},
        tmp2 = {},
        postData = {},
        editable, k, fr, resp, cv, ind = $self.jqGrid("getInd", rowid, true),
        $tr = $(ind),
        opers = p.prmNames,
        errcap = getRes("errors.errcap"),
        bClose = getRes("edit.bClose"),
      ...
      if (editable === "1") {
        ...
        if (!isRemoteSave) {
          ...
          for (k = 0; k < p.savedRow.length; k++) {
            if (String(p.savedRow[k].id) === String(oldRowId)) {
              fr = k;
              break;
            }
          }
          ...
          $.ajax($.extend({
            ...
            error: function(res, stat, err) {
              ...
              if ($.isFunction(o.errorfunc)) {
                ...
                try {
                  infoDialog.call($t, errcap, '<div class="' + getGuiStateStyles.call($t, "error") + '">' + rT + "</div>", bClose, {
                    buttonalign: "right"
                  });
                } catch (e) {
                  alert(rT);
  ...
  if ($UiMultiselect) {
    if ($UiMultiselect.prototype._setSelected) {
      ...
      $UiMultiselect.prototype._setSelected = function(item, selected) {
        var self = this,
          ret = setSelected.call(self, item, selected);
    ...
    columnChooser: function(opts) {
      ...
      opts = $.extend({
          ...
          apply_perm: function() {
            ...
            //fixedCols.slice(0);
            $("option", select).filter(":selected").each(function() {
            ...
            $self.jqGrid("setGridWidth", !p.autowidth && (p.widthOrg === undefined || p.widthOrg === "auto" || p.widthOrg === "100%") ? p.tblwidth : p.width,
              p.shrinkToFit);
          },
    ...
    gridResize: function(opts) {
      return this.each(function() {
        var $t = this,
          grid = $t.grid,
          p = $t.p,
          bdivSelector = p.gView + ">.ui-jqgrid-bdiv",
          onlyHorizontal = false,
          sel, gridHeight = p.height;
  ...
  function ArrayOfFieldsets(trimByCollect, caseSensitive, skipSort, dimension, fieldName) {
    var iField, dimensionLength = dimension.length,
      dimensionItem, self = this,
  ...
  ArrayOfFieldsets.prototype.createDataIndex = function(data) {
    var self = this,
      iRow, nRows = data.length,
      fieldLength = self.fieldLength,
      values, v,
      fieldNames = self.fieldNames,
      indexesOfSourceData = self.indexesOfSourceData,
      iField, compareResult, i, item,
      items = self.items,
      iMin, iMax;
  ...
  jgrid.extend({
    pivotSetup: function(data, options) {
      ...
      var self = this[0],
        ...
        row, i, k, nRows = data.length,
        x, y, cm, iRow, cmName, iXData, itemXData, pivotInfos, rows,
        ...
        colModel = [],
        outputItems = [],
        aggrContextTotalRows = new Array(aggrlen),
        aggrContextGroupTotalRows = new Array(ylen),
        ...
        finalizeGroupTotals = function(iyData, itemYData, previousY, iAggr) {
          var iLevel, level = yIndex.getIndexOfDifferences(itemYData, previousY),
            fieldName, aggrGroup;
      ...
      for (i = 0; i < xlen; i++) {
        ...
        cm = {
          name: "x" + i,
          label: x.label != null ?
            ($.isFunction(x.label) ? x.label.call(self, x, i, o) : x.label) : x.dataName,
  ...
  jgrid.extend({
    ...
    addSubGridCell: function(pos, iRow, rowid, item) {
      var self = this[0],
        subGridOptions = self.p.subGridOptions,
        hasSubgrid = $.isFunction(subGridOptions.hasSubgrid) ?
        subGridOptions.hasSubgrid.call(self, {
          rowid: rowid,
          iRow: iRow,
          iCol: pos,
          data: item
  ...
  */
  ...
  window.tableToGrid = function(selector, options) {
    $(selector).each(function() {
      var self = this,
        $self = $(this),
        w, inputCheckbox, inputRadio, selectMultiple, selectSingle, selectable, a, id,
        colModel = [],
        colNames = [],
        data = [],
        rowIds = [],
  ...
  jgrid.extend({
    ...
    expandRow: function(record) {
      this.each(function() {
        ...
        var expanded = p.treeReader.expanded_field,
          rowid = record[p.localReader.id]; // without prefix
        if (!treeGridFeedback.call($t, "beforeExpandRow", {
            rowid: rowid,
            item: record
          })) {
          return;
        ...
        treeGridFeedback.call($t, "afterExpandRow", {
          rowid: rowid,
          item: record
        });
      });
    ...
    collapseRow: function(record) {
      this.each(function() {
        ...
        var expanded = p.treeReader.expanded_field,
          rowid = record[p.localReader.id]; // without prefix
        if (!treeGridFeedback.call($t, "beforeCollapseRow", {
            rowid: rowid,
            item: record
          })) {
          return;
        ...
        treeGridFeedback.call($t, "afterCollapseRow", {
          rowid: rowid,
          item: record
        });
      });
    ...
    getNodeParent: function(rc) {
      ...
      var p = $t.p,
        treeReader = p.treeReader,
        parentIdName = treeReader.parent_id_field,
    ...
    getNodeAncestors: function(rc) {
      ...
      this.each(function() {
        var $t = this,
          $self = $($t),
          getNodeParent = base.getNodeParent;
    ...
    isVisibleNode: function(rc) {
      ...
      this.each(function() {
        ...
        var ancestors = base.getNodeAncestors.call($($t), rc),
    ...
    expandNode: function(rc) {
      return this.each(function() {
        ...
        if (!rc[expanded]) {
          ...
          if (!treeGridFeedback.call($t, "beforeExpandNode", {
              rowid: id,
              item: rc
            })) {
            return;
          ...
          treeGridFeedback.call($t, "afterExpandNode", {
            rowid: id,
            item: rc
          });
        }
    ...
    collapseNode: function(rc) {
      return this.each(function() {
        ...
        if (rc[expanded]) {
          var id = getAccessor(rc, p.localReader.id);
          if (!treeGridFeedback.call($t, "beforeCollapseNode", {
              rowid: id,
              item: rc
            })) {
            return;
    ...
    SortTree: function(sortname, newDir, st, datefmt) {
      return this.each(function() {
        ...
        var i, len, rec, records = [],
          rt = base.getRootNodes.call($self),
    ...
    collectChildrenSortTree: function(records, rec, sortname, newDir, st, datefmt) {
      return this.each(function() {
        ...
        var i, len, child, ch = base.getNodeChildren.call($self, rec),
    ...
    delTreeNode: function(rowid) {
      return this.each(function() {
        var $t = this,
          p = $t.p,
          myright, width, res, key, rid = p.localReader.id,
          i, $self = $($t),
    ...
    addChildNode: function(nodeid, parentid, data, expandData) {
      ...
      if (data) {
        ...
        var method, parentindex, parentdata, parentlevel, i, len, max = 0,
          rowind = parentid,
          leaf, maxright,
          expanded = p.treeReader.expanded_field,
          isLeaf = p.treeReader.leaf_field,
        ...
        if (nodeid == null) {
          ...
          if (i >= 0) {
            while (i >= 0) {
              max = Math.max(max, parseInt(p.data[i][p.localReader.id], 10));
              i--;
            }
          }
          nodeid = max + 1;
  ...
  var namespace = ".jqGrid",
    ...
    getMouseCoordinates = function(e) {
      var orgEvent = e.originalEvent,
        touches = orgEvent.targetTouches;
      if (touches) {
        touches = touches[0];
        return {
          x: touches.pageX,
  ...
  $FnFmatter.select = function(cellval, opts) {
    var ret = [],
      ...
      delim = op.delimiter || ";";
    if (oSelect) {
      var msl = op.multiple === true ? true : false,
        scell = [],
        sv,
        mapFunc = function(n, i) {
          if (i > 0) {
            return n;
          }
        };
  ...
  $FnFmatter.select.getCellBuilder = function(opts) {
    ...
    var colModel = opts.colModel,
      ...
      isMultiple = op.multiple === true ? true : false,
      sv, so, i, nOpts, selOptions = {},
      mapFunc = function(n, i) {
        if (i > 0) {
          return n;
        }
  ...
  $FnFmatter.actions = function(cellval, opts, rwd, act) {
    var rowid = opts.rowId,
      ...
      p = $t.p,
      $self = $($t),
      i, customAction, info, displayMask = {},
  ...
  $FnFmatter.actions.pageFinalization = function(iCol) {
    var $self = $(this),
      ...
      showHideEditDelete = function(show, rowid) {
        var maxfrozen = 0,
          tr, $actionsDiv, len = colModel.length,
          i;
        for (i = 0; i < len; i++) {
          // from left, no breaking frozen
          if (colModel[i].frozen !== true) {
  ...
  $.unformat = function(cellval, options, pos, cnt) {
    ...
    var ret, colModel = options.colModel,
      formatType = colModel.formatter,
      p = this.p,
